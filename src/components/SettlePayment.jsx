...
function SettlePayment() {
  ...
  const fetchSettlements = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/settlements?tripOrganizer=${trip.trip_organizer}&tripId=${trip.trip_id}`);
  ...
  const incrementAmount = () => {
    setAmount(prev => Math.min(typeof prev === 'number' ? prev + 100 : 100, selectedSettlement.user_spending));
  };
