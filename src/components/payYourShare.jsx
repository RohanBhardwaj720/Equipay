...
function PayYourShare(props) {
  ...
  const fetchSpending = async (userId, tripId) => {
    try {
      const response = await axios.get('http://localhost:3001/spending', {
  ...
  const fetchTripOrganizer = async () => {
    try {
        const response = await axios.get(`http://localhost:3001/user/${trip.trip_organizer}`);
  ...
  const handleConfirmation = async (confirmed) => {
    ...
    if (confirmed) {
      try {
        await axios.patch(`http://localhost:3001/pay/${trip.trip_id}`, {
          money: amount,
  ...
  const incrementAmount = () => {
    setAmount(prev => Math.min(typeof prev === 'number' ? prev + 100 : 100, (money>0)? 0 : (Math.abs(money))));
  };
    ...
    <div className={styles.paymentContainer}>
      ...
      {showUPICard && (
        <div className={styles.upiOverlay}>
          ...
          />
          <button className={styles.closeButton} onClick={handleUPICardClose}>Close</button>
        </div>
      )}
